...
class AutonomousGoalAgent:
    ...
    def create_plan(self) -> list[PlanStep]:
        ...
        if any(word in text for word in ("so sánh", "shortlist", "tốt nhất")):
            plan.append(
                {
                    "step": len(plan) + 1,
        ...
        if any(word in text for word in ("lịch", "khung giờ", "xem nhà", "đặt")):
            plan.append(
                {
                    "step": len(plan) + 1,
    ...
    def execute(self, step_runner: StepRunner | None = None) -> dict[str, Any]:
        ...
        requires_confirmation = _requests_booking(self.goal.casefold())
        return {
            "goal": self.goal,
            "plan": deepcopy(plan),
...
def _requests_booking(text: str) -> bool:
    return "đặt lịch" in text or "book" in text or "xác nhận lịch" in text
