...
export function ConfirmationModal({
  ...
  const hasConfirmationToken = Boolean(slot.confirmation_token?.trim());
  ...
  const submit = (event: FormEvent) => {
    ...
    const confirmationToken = slot.confirmation_token;
    ...
    if (!normalizedPhone) nextErrors.phone = "Nhập số điện thoại Việt Nam hợp lệ.";
        ...
        <form className="confirmation-form" onSubmit={submit}>
          {!hasConfirmationToken ? (
            <div className="privacy-note" role="alert">
              ...
              Khung giờ đã thiếu mã xác nhận. Hãy đóng hộp này và tra lịch lại.
            </div>
          ) : null}
          <label htmlFor="viewer-name">
            ...
            <input
              autoComplete="name"
              autoFocus
              disabled={submitting || !hasConfirmationToken}
              id="viewer-name"
