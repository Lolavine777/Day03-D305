...
describe("RentMate UI", () => {
  ...
  it("render hồ sơ, chọn slot và chỉ gửi booking sau modal xác nhận", async () => {
            ...
            slots: [
              {
                ...
                available: true,
                confirmation_token: "confirm-token-SL-001"
              }
            ]
          });
        }
    ...
    await user.type(
      within(dialog).getByLabelText(/Số điện thoại/),
      "(0912) 345/678"
    );
    await user.click(within(dialog).getByRole("button", { name: "Xác nhận đặt lịch" }));
  ...
  it("không cho đặt lịch khi slot thiếu confirmation token và hướng dẫn tra lịch lại", async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL, _init?: RequestInit) => {
      const path = requestPath(input);
      if (path.endsWith("/api/sessions")) return jsonResponse({ session_id: "session-test" }, 201);
      if (path.endsWith("/api/chat")) {
        return jsonResponse({
          ...baseChatResponse,
          answer: "Có một lịch xem nhưng phản hồi không có token.",
          properties: [
            {
              property_id: "HN-CG-001",
              title: "Phòng sáng gần Đại học Quốc gia",
    ...
    await screen.findByText("Có một lịch xem nhưng phản hồi không có token.");
    await user.click(screen.getByRole("button", { name: /14:00/ }));

    expect(screen.queryByRole("dialog")).not.toBeInTheDocument();
    expect(screen.getByRole("alert")).toHaveTextContent(/tra lịch lại/i);
    const chatCalls = fetchMock.mock.calls.filter(([input]) =>
      requestPath(input as RequestInfo | URL).endsWith("/api/chat")
    );
  ...
  it("đóng modal và xóa slot cũ khi confirmation token hết hạn", async () => {
    let chatCount = 0;
    const fetchMock = vi.fn(async (input: RequestInfo | URL, _init?: RequestInit) => {
      const path = requestPath(input);
      if (path.endsWith("/api/sessions")) {
        return jsonResponse({ session_id: "session-test" }, 201);
      }
      if (path.endsWith("/api/chat")) {
        chatCount += 1;
        if (chatCount === 1) {
            ...
            slots: [
              {
                ...
                starts_at: "2026-08-01T14:00:00+07:00",
                available: true,
                confirmation_token: "expired-confirmation-token-SL-001"
              }
            ]
          });
        }
        return jsonResponse(
          {
            detail: {
              ...
              message: "Mã xác nhận đã hết hạn. Hãy tra lịch và chọn lại khung giờ."
            }
          },
          403
        );
    ...
    await screen.findByText("Đã tìm thấy một lịch.");
    await user.click(screen.getByRole("button", { name: /14:00/ }));

    const dialog = screen.getByRole("dialog", {
      name: "Kiểm tra trước khi đặt lịch"
    });
    await user.type(within(dialog).getByLabelText(/Người đi xem/), "Nguyễn An");
    await user.type(within(dialog).getByLabelText(/Số điện thoại/), "0912345678");
    await user.click(
      within(dialog).getByRole("button", { name: "Xác nhận đặt lịch" })
    );

    await waitFor(() => expect(screen.queryByRole("dialog")).not.toBeInTheDocument());
    expect(screen.getByRole("alert")).toHaveTextContent(/hết hạn.*tra lịch/i);
